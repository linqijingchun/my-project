...
- GUI 支持自然语言输入、K 条路径查询、约束路径查询、拓扑分析可视化
...
- 拓扑可视化（节点可拖拽调整布局）
- 添加有向边/无向边
- 查询最短路径、所有最短路径、K 条最短路径
- 可达性判断
- 约束路径查询（via/avoid/hops）
- 拓扑分析可视化（关键节点橙色高亮、瓶颈链路橙色加粗）
- 自然语言输入框（支持所有命令和自然语言）
- 优化策略切换（自动刷新边标签和路径高亮）
- 加载和保存拓扑文件
- 清空当前图
...
- 修复瓶颈评分公式：passCount=0 的边不再恒为 0 分，改为按权重排序
- 修复拓扑文件编码兼容：自动检测 UTF-8/GBK 编码，旧 GBK 文件可正常加载
- 新增 4 个测试用例（总计 67 个）

### 1.5 版本 — GUI 可视化增强

- GUI 新增自然语言输入框，支持所有标准命令和自然语言交互
- GUI 新增 K 条最短路径查询（K 值输入框 + kpath 按钮）
- GUI 新增约束路径查询面板（via/avoid/hops 输入）
- GUI 新增拓扑分析可视化（关键节点橙色、瓶颈链路橙色加粗）
- 边标签改为显示当前策略下的代价值，策略切换后自动刷新
- 策略下拉框与自然语言命令双向同步
- 节点支持鼠标拖拽调整布局
- 右侧面板改为可滚动布局，避免小屏幕溢出
- 演示拓扑边附带四维指标，策略切换有直观效果
- `AgentResponse` 新增 `analyzeResult` 结构化字段
- `handleKPath` 返回值修复为携带路径列表（`successWithPaths`）
- `handleAnalyze` 返回值修复为携带结构化分析结果（`successWithAnalyze`）
...
当前 P0-P5 已全部完成，已知问题已修复。后续可考虑的方向：

- 增加示例拓扑文件和运行截图
- 预留 LLM 接入，让大模型负责复杂自然语言理解
- 真实网络环境对接（SNMP/NETCONF 数据采集）
